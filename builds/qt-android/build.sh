...
##
# Build libzsync from local source

(android_build_verify_so "liblibzsync.so" "libczmq.so" "libzyre.so" &> /dev/null) || {
    rm -rf "${cache}/libzsync"
    (cp -r ../.. "${cache}/libzsync" && cd "${cache}/libzsync" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/libzsync" && ./autogen.sh \
...
android_build_verify_so "liblibzsync.so" "libczmq.so" "libzyre.so"
